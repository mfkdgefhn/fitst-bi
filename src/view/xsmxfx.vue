<template>
  <div>
    <div class="select">
      ...
      <div class="block">
        <span class="demonstration">款号:</span>
...
export default {
  ...
  methods: {
    ...
    getAjax () {
      ...
      let vEname = JSON.stringify(this.product).replace(/"/g, '').replace(/\[/g, '').replace(/\]/g, '')
      if (vEname !== '') {
        url = url + '&vEname=' + vEname
      }
    ...
    getproduct () {
      ...
      let vSeason = this.vseason.length === 0 ? '37,38,39,40' : JSON.stringify(this.vseason).replace(/"/g, '').replace(/\[/g, '').replace(/\]/g, '')
      let vYear = this.productYear
      let url = 'http://10.10.1.41/Q/r.do?o=sykh' + '&vSotype=' + vSotype + '&vSeason=' + vSeason + '&vYear=' + vYear
      console.log(url)
      let successCallback = (response) => {
        let vproduct = response.data

        this.states = []
        vproduct.forEach(element => {
          this.states.push(element.eName)
        })
        // for (let i = 0; i < 10; i++) {
        //   this.states.push(vproduct[i].eName)
        // }
      }
      let errorCallback = (response) => {
      }
